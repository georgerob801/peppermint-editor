...
void AssetManager::saveAssetFile() {
	std::ofstream assetFile(this->assetFileAsset->path.c_str(), std::ios::out | std::ios::binary);
...
void AssetManager::loadAssetFile(Asset* assetFileIn) {
	...
	std::ifstream assetFile(assetFileIn->path.c_str(), std::ios::binary | std::ios::in);
