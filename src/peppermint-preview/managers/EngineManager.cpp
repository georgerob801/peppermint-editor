...
void EngineManager::editorLoopIteration() {
	for (unsigned int i = 0; i < EngineManager::getWM()->gameObjects.size(); i++) {
		for (unsigned int j = 0; j < EngineManager::getWM()->gameObjects[i]->components.size(); j++) {
			EngineManager::getWM()->gameObjects[i]->components[j]->editorLoop();
		}
	}
}
...
void EngineManager::saveGameFile() {
	...
	std::ofstream gameFileFS(EngineManager::gameFile->path.c_str(), std::ios::binary | std::ios::trunc);
...
void EngineManager::loadFromGameFile() {
	std::ifstream gameFile(EngineManager::gameFile->path.c_str(), std::ios::binary | std::ios::in);
	...
	assetManagerAsset->path = assetFilePath;
