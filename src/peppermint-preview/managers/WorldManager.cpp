...
void WorldManager::populateRenderQueue(Window* window) {
	for (unsigned int i = 0; i < this->gameObjects.size(); i++) {
		if (this->stopProcessingWorld) break;
		if (!this->gameObjects[i]->isEnabled()) continue;
		for (unsigned int j = 0; j < this->gameObjects[i]->components.size(); j++) {
...
void WorldManager::saveWorldFile() {
	std::ofstream worldFile(this->worldAsset->path.c_str(), std::ios::binary | std::ios::trunc);
...
void WorldManager::loadWorldFile() {
	std::ifstream worldFile(this->worldAsset->path.c_str(), std::ios::binary | std::ios::in);
