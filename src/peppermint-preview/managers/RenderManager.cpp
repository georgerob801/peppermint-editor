...
void RenderManager::resizeFBOAttachments() {
	if (this->FBO != NULL) {
		...
		// ✨ optimisation ✨
		//if (size[0] == this->lastWidth && size[1] == this->lastHeight) return;
		//this->lastWidth = size[0];
		//this->lastHeight = size[1];
...
void RenderManager::renderFrame(bool useProjection, mat4 proj) {
	...
	while (this->activeRenderQueue->renderItems.size() != 0) {
		...
		if (!useProjection) {
			...
			this->activeRenderQueue->renderItems[0].shader->setMat4f((char*)"projection", proj);
		}
		...
		this->activeRenderQueue->renderItems[0].shader->setBool((char*)"outlineMode", false);
		this->activeRenderQueue->renderItems[0].shader->setVec3f((char*)"tintColour", vec3(1.0f, 1.0f, 1.0f));

		if (this->toHighlight == this->activeRenderQueue->renderItems[0].go) {
			this->activeRenderQueue->renderItems[0].shader->setVec3f((char*)"tintColour", this->selectionTintColour);
		...
		if (this->toHighlight == this->activeRenderQueue->renderItems[0].go) {
			...
			this->activeRenderQueue->renderItems[0].shader->setBool((char*)"outlineMode", true);
			this->activeRenderQueue->renderItems[0].shader->setVec3f((char*)"outlineColour", vec3(1.0f, 1.0f, 1.0f));
			this->activeRenderQueue->renderItems[0].shader->setFloat((char*)"outlineAlpha", 0.6f);
