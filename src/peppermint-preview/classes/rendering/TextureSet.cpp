...
TextureSet::TextureSet() : Asset(AssetType::TEXTURE_SET) {
	this->displayTypeName = "texture set";

	this->clearParameters();
	this->setupDisplayTypeEnum();

	auto settings = this->registerParameter("textures", &this->textures, ASSETV);
	settings->requiresEnumValue = true;
	settings->requiredEnumValue = Asset::TEXTURE;
}
...
vector<byte> TextureSet::serialise() {
	...
	for (unsigned int i = 0; i < this->textures.size(); i++) {
		void* sID = nullptr;
		byte* textureIDB = reinterpret_cast<byte*>(this->textures[i]->getSerialisedID(&sID));
