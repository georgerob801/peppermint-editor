...
Texture::Texture() : Asset(AssetType::TEXTURE) {
	...
	this->displayTypeName = "texture";

	this->clearParameters();
	this->setupDisplayTypeEnum();

	auto settings = this->registerParameter("image", &this->imageAsset, editor::gui::ComponentParameterRegister::ASSET);
	settings->requiresEnumValue = true;
	settings->requiredEnumValue = peppermint::Asset::IMAGE;
}
...
void Texture::loadFromAsset() {
	char* path = (char*)this->imageAsset->path.c_str();
...
vector<byte> Texture::serialise() {
	...
	void* sID = nullptr;
	byte* imageAssetID = reinterpret_cast<byte*>(this->imageAsset->getSerialisedID(&sID));
	for (unsigned int i = 0; i < sizeof(void*); i++) {
