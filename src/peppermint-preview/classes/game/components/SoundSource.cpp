...
SoundSource::SoundSource() {
	...
	this->displayTypeName = "sound source";
	this->registerParameter("sound", &this->sound, editor::gui::ComponentParameterRegister::ASSET);
	auto settings = this->registerParameter("pitch", &this->pitch, FLOAT);
	(*settings).setInRange(FLT_MIN, FLT_MAX);
	settings = this->registerParameter("gain", &this->gain, FLOAT);
	(*settings).setInRange(FLT_MIN, FLT_MAX);
	this->registerParameter("use doppler", &this->useDoppler, BOOL);
	this->registerParameter("play by default", &this->playByDefault, BOOL);
	this->registerParameter("loop", &this->loopSound, BOOL);

	this->description = "plays sounds that can be heard using a sound listener component";
	this->iconIndex = 2;
}
...
vector<byte> SoundSource::serialise() {
	...
	void* sID = nullptr;
	toAdd = reinterpret_cast<byte*>(this->sound->asset->getSerialisedID(&sID));
	for (unsigned int i = 0; i < sizeof(void*); i++) {
		out.push_back(toAdd[i]);
	}
