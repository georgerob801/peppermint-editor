...
PlayerController::PlayerController() {
	this->displayTypeName = "player controller";

	this->type = Component::PLAYER_CONTROLLER;

	this->registerParameter("speed", &this->speed, FLOAT);
	auto settings = this->registerParameter("snap range", &this->snapRange, FLOAT, false, true);
	(*settings).setInRange(0, FLT_MAX);
	settings = this->registerParameter("navigable map", &this->navMap, editor::gui::ComponentParameterRegister::COMPONENT);
	(*settings).requiredTypeDisplayName = "navigable map";
	settings = this->registerParameter("warp cooldown", &this->warpCooldown, DOUBLE, false, true);
	(*settings).setInRange(0, FLT_MAX);

	this->description = "basic player controller that can navigate a navigable map";
	this->iconIndex = 6;
}
...
vector<byte> PlayerController::serialise() {
	...
	void* sID = nullptr;
	byte* navMapB = reinterpret_cast<byte*>(this->navMap->getSerialisedID(&sID));
	for (unsigned int i = 0; i < sizeof(void*); i++) {
