...
Transform::Transform() {
	this->displayTypeName = "transform";
	...
	this->registerParameter("position", &this->position, FLOATV3);
	auto settings = this->registerParameter("rotation", &this->rotation, FLOATV3, true);
	// in range is visible in the editor
	(*settings).setInRange(0.0f, 360.0f);
	// out range is what the value is actually set to
	(*settings).setOutRange(0, two_pi<float>());
	this->registerParameter("scale", &this->scale, FLOATV3);

	this->description = "handles the position of game objects";
	this->iconIndex = 7;
}
...
vector<byte> Transform::serialise() {
	...
	void* tempNull = nullptr;
	void* sID = nullptr;
	byte* parentIDB = reinterpret_cast<byte*>(this->parent != nullptr ? this->parent->getSerialisedID(&sID) : &tempNull);
	for (unsigned int i = 0; i < sizeof(void*); i++) {
