...
AnimatedTilesetRenderer::AnimatedTilesetRenderer(unsigned int width, unsigned int height) : TilesetRenderer(width, height) {
	...
	this->displayTypeName = "animated tileset renderer";
	this->registerParameter("animations", &this->animations, ASSETV);
	this->registerParameter("running", &this->running, BOOL);

	this->registerParameter("tileset asset", &this->tileset, editor::gui::ComponentParameterRegister::ASSET);

	this->description = "renders an animated tileset";
}
...
vector<byte> AnimatedTilesetRenderer::serialise() {
	...
	void* sID = nullptr;
	byte* tilesetIDB = reinterpret_cast<byte*>(this->tileset->getSerialisedID(&sID));
	for (unsigned int i = 0; i < sizeof(void*); i++) {
	...
	for (unsigned int i = 0; i < this->animations.size(); i++) {
		void* sID = nullptr;
		byte* animationIDB = reinterpret_cast<byte*>(this->animations[i]->getSerialisedID(&sID));
