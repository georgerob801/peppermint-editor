...
TextRenderer::TextRenderer() {
	...
	this->displayTypeName = "text renderer";

	this->registerParameter("text", &this->text, STRING);

	this->registerParameter("font file", &this->fontFile, editor::gui::ComponentParameterRegister::ASSET);
	this->registerParameter("pixel size", &this->pixelSize, UINT);
	this->registerParameter("x spacing", &this->xSpacing, FLOAT);
	this->registerParameter("y spacing", &this->ySpacing, FLOAT);

	this->registerParameter("auto-wrap x", &this->autoWrapX, BOOL);
	auto settings = this->registerParameter("max width", &this->maxWidth, FLOAT);
	(*settings).tooltip = "only applies when auto-wrap x is enabled";

	this->description = "renders text";
}
...
void TextRenderer::editorLoop() {
	if (this->lastText != this->text) {
		this->setText(this->text);
		this->lastText = this->text;
	}

	if (this->lastPixelSize != this->pixelSize || this->lastXSpacing != this->xSpacing || this->lastYSpacing != this->ySpacing || this->lastAutoWrapX != this->autoWrapX || this->lastMaxWidth != this->maxWidth) {
		this->lastPixelSize = this->pixelSize;
		this->lastXSpacing = this->xSpacing;
		this->lastYSpacing = this->ySpacing;
		this->lastAutoWrapX = this->autoWrapX;
...
void TextRenderer::generateVertices() {
	...
	if (this->fontFile != this->lastFontFile) {
		this->lastFontFile = this->fontFile;
...
vector<byte> TextRenderer::serialise() {
	...
	void* sID = nullptr;
	toAdd = reinterpret_cast<byte*>(this->fontFile->getSerialisedID(&sID));
	for (unsigned int i = 0; i < sizeof(void*); i++) {
		out.push_back(toAdd[i]);
	}
