...
namespace peppermint {
	namespace game {
		...
		/// </summary>
		class GameObject : public Serialisable, public Toggleable, public editor::gui::GUIInfo {
		public:
			/// <summary>
			...
			/// <summary>
			/// Get a Component of a specified type from this GameObject.
			/// </summary>
			/// <typeparam name="T"></typeparam>
			/// <returns></returns>
			template<class T> T* getComponent() {
				for (unsigned int i = 0; i < this->components.size(); i++) {
					T* test = dynamic_cast<T*>(this->components[i]);
					if (test) return test;
				}
			}
