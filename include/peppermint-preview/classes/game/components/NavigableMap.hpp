		...
		namespace components {
			...
			public:
				...
				NavigableMap(unsigned int width, unsigned int height) {
					...
					this->displayTypeName = "navigable map";
				
					this->description = "stores information about which tiles a player can move on";
					this->iconIndex = 5;
				}
				...
				vector<byte> serialise() {
					...
					for (unsigned int i = 0; i < this->warpTiles.size(); i++) {
						void* warpID = nullptr;
						byte* warpIDB = reinterpret_cast<byte*>(this->warpTiles[i]->getSerialisedID(&warpID));
